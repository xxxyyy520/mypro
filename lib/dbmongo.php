...
class dbmongo {
    ...
    * @return array
    */
  	public function fetchPage($colName,$query=array(),$fields=array(),$sort=array(),$nums='20',$type='0')
  	{
        //得到总数量
    		...
    		//print $query;
        $record	 = $this->select($colName,$query,$fields,$sort,$nums,$offset);
        if($record){
            $data["record"] = $record;
    ...
    public function fetchDistinct($colName,$query=array(),$fields="_id")
    {
        // 得到集合名
        $col = $this->_getCol($colName);
        // 处理结果集
